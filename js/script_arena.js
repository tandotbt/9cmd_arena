...
function sortColumn(colIdx) {
  const SHOW_BXH_MAX = parseInt($('#setRankDisplay').val()) || 3000;

  // Dữ liệu gốc
  var data = getDataFromSessionStorage("temp", "dataArena");

  // Kiểm tra nếu data có giá trị
  if (!data || data.length === 0) {
    console.error("Dữ liệu không hợp lệ hoặc không tồn tại");
    return;
  }

  // Mảng các key tương ứng với các cột trong bảng
  const colId = ['rankid', 'portraitId', 'avatarname', 'cp', 'score', 'stake', 'win', 'currenttickets', 'purchasedTicketCount', 'purchasedTicketCountOld', 'purchasedTicketNCG'];

  // Kiểm tra trạng thái sắp xếp cũ từ sessionStorage
  let sortOrder = sessionStorage.getItem(`sortOrder_${colIdx}`); // Lấy trạng thái sắp xếp cũ

  // Nếu chưa có trạng thái, mặc định là "ascending"
  if (!sortOrder) {
    sortOrder = 'ascending';
  }
  ...
  data.sort(function (a, b) {
    // Lấy key của cột dựa trên mảng colId
    var key = colId[colIdx];
    var aValue = a[key];
    var bValue = b[key];

    // So sánh dữ liệu theo hướng đã lưu trong sessionStorage
    if (typeof aValue === 'number' && typeof bValue === 'number') {
      ...
      if (sortOrder === 'ascending') {
        ...
        return 0; // Nếu chưa sắp xếp, trả về không thay đổi
      }
    } else {
      // Nếu là chuỗi
      if (sortOrder === 'ascending') {
        return aValue.toString().localeCompare(bValue.toString()); // Tăng dần
      } else if (sortOrder === 'descending') {
        return bValue.toString().localeCompare(aValue.toString()); // Giảm dần
      } else {
        return 0; // Nếu chưa sắp xếp, trả về không thay đổi
      }
    }
  ...
  // Cập nhật lại trạng thái sắp xếp trong sessionStorage
  if (sortOrder === 'ascending') {
    // Đổi sang giảm dần khi người dùng click lần 2
    sessionStorage.setItem(`sortOrder_${colIdx}`, 'descending');
  } else if (sortOrder === 'descending') {
    // Nếu đang giảm dần, quay lại trạng thái ban đầu
    sessionStorage.setItem(`sortOrder_${colIdx}`, 'none');
  } else {
    // Nếu chưa sắp xếp, đổi sang tăng dần
    sessionStorage.setItem(`sortOrder_${colIdx}`, 'ascending');
  }

  // Sau khi sắp xếp, gọi lại hàm creatTableArena để tạo bảng mới
        ...
        if (Array.isArray(apiData2) && apiData2.length > 0) {
          ...
          const mergedData = apiData1.map(data1 => {
            ...
            return {
              ...
              agentaddress: matchingData2?.agentaddress || "",
            };
          });
